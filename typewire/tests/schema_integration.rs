...
/// Convert a `Copy` value to its raw byte representation.
const fn as_bytes<T: Copy>(val: &T) -> &[u8] {
  // SAFETY: `T: Copy` guarantees no padding/drop concerns; the pointer is valid
  // for `size_of::<T>()` bytes and the lifetime is tied to `val`.
  unsafe {
    core::slice::from_raw_parts(
      std::ptr::from_ref::<T>(val).cast::<u8>(),
      core::mem::size_of::<T>(),
    )
  }
}
...
#[test]
fn box_ident_wraps_inner() {
  let ident = <Box<f64> as Typewire>::IDENT;
  let bytes = as_bytes(&ident);
  // BoxIdent<PrimitiveIdent> layout: [Tag::Box=3][Tag::Primitive=1][Scalar::f64=14]
  assert_eq!(bytes, &[3, 1, 14]);
}
